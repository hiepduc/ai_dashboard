...
function Slider() {
  ...
  useEffect(() => {
    ...
    if (markerChartElement) {
      ...
      // Determine the length of historical data
      const historicalDataLength = forecastChart.data.datasets[0].data.length;

      // Calculate the position of the vertical line based on the current slider value
      const lineAnnotationValue = historicalDataLength + sliderValue;
      // const lineAnnotationValue =        sliderValue + Number(selectedOptions.timeScopes);
